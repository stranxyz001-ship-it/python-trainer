...
TASKS_BY_LEVEL: Dict[int, List[Dict[str, Any]]] = {
    ...
    4: [
        {"id": "l4_group_anagrams", "title": "Group Anagrams", "prompt": "Write `group_anagrams(words)` grouping anagrams.", "function_name": "group_anagrams", "tests": [{"input": [["eat", "tea", "tan", "ate", "nat", "bat"]], "expected": [["eat", "tea", "ate"], ["tan", "nat"], ["bat"]]}], "hint": "Use sorted words as dictionary keys."},
        {"id": "l4_two_sum", "title": "Two Sum Indices", "prompt": "Write `two_sum(nums, target)` returning two indices or None.", "function_name": "two_sum", "tests": [{"input": [[2, 7, 11, 15], 9], "expected": (0, 1)}, {"input": [[3, 2, 4], 6], "expected": (1, 2)}], "hint": "Map values to indices."},
    ],
    5: [
        {"id": "l5_merge_intervals", "title": "Merge Intervals", "prompt": "Write `merge_intervals(intervals)` merging overlapping intervals.", "function_name": "merge_intervals", "tests": [{"input": [[[1, 3], [2, 6], [8, 10], [15, 18]]], "expected": [[1, 6], [8, 10], [15, 18]]}, {"input": [[[1, 4], [4, 5]]], "expected": [[1, 5]]}], "hint": "Sort by start and merge as you go."},
...
def init_sqlite(path: str = DB_PATH) -> sqlite3.Connection:
    ...
    if conn.execute("SELECT COUNT(*) FROM user_state").fetchone()[0] == 0:
        s = default_state(); conn.execute("INSERT INTO user_state VALUES (1,?,?,?,?,?,?)", tuple(s.values()))
...
def init_json(path: str = JSON_PATH) -> Dict[str, Any]:
    try:
        ...
        data.setdefault("user_state", default_state()); data.setdefault("attempts", []); return data
...
class Storage:
    def __init__(self, mode: str = DEFAULT_STORAGE):
        ...
        if self.mode == "sqlite": self.conn = init_sqlite()
        else: self.json_path, self.data = JSON_PATH, init_json()
    ...
    def save_json(self) -> None:
        with open(self.json_path, "w", encoding="utf-8") as f: json.dump(self.data, f, indent=2)

    def update_state(self, **changes: Any) -> None:
        state = default_state(); state.update(self.get_state()); state.update(changes)
        keys = ("level", "streak", "last_task_date", "last_task_id", "success_streak", "preferred_name")
        if self.mode == "sqlite":
            self.conn.execute("UPDATE user_state SET level=?,streak=?,last_task_date=?,last_task_id=?,success_streak=?,preferred_name=? WHERE id=1", tuple(state[k] for k in keys)); self.conn.commit()
        else: self.data["user_state"] = state; self.save_json()
...
def choose_daily_task(level: int) -> Dict[str, Any]:
    tasks = TASKS_BY_LEVEL.get(int(level), TASKS_BY_LEVEL[1]); seed = hashlib.sha256(f"{dt.date.today()}{APP_SECRET}{level}".encode()).hexdigest()
...
DISALLOWED_NODES = tuple(getattr(ast, n) for n in _NODE_NAMES if hasattr(ast, n))
...
SAFE_BUILTINS = {n: getattr(builtins, n) for n in ("abs", "all", "any", "bool", "chr", "dict", "float", "int", "len", "list", "max", "min", "pow", "range", "str", "sum", "enumerate", "sorted", "set", "tuple", "map", "filter", "zip", "isinstance")}
...
def ast_safety_check(code: str) -> Tuple[bool, str]:
    ...
    except (SyntaxError, ValueError, TypeError) as exc: return False, f"Syntax error: {exc}"
...
def worker(code: str, name: str, tests: List[Dict[str, Any]], result_queue: Any) -> None:
    try:
        namespace = {"__builtins__": SAFE_BUILTINS.copy()}; exec(compile(code, "<student_code>", "exec"), namespace, namespace)
        func = namespace.get(name)
        if not callable(func): result_queue.put({"error": f"Function `{name}` not found or not callable."}); return
        results = []; all_passed = True
        for test in tests:
            try:
                inp, expected = test["input"], test["expected"]; output = func(*inp) if isinstance(inp, (list, tuple)) else func(inp); passed = equal_output(output, expected)
                item = {"input": inp, "expected": expected, "output": output, "passed": passed}
            except Exception as exc:
                passed = False; item = {"input": test.get("input"), "expected": test.get("expected"), "output": f"Error: {exc}", "passed": False}
...
def main() -> None:
    ...
    st.markdown("""
    <style>
    .dashboard-hero { padding: 1.35rem 1.6rem; border-radius: 20px; color: white; background: linear-gradient(120deg,#6d28d9,#db2777 52%,#f97316); box-shadow: 0 8px 24px rgba(109,40,217,.25); margin-bottom: 1rem; }
    .dashboard-hero h1 { margin: 0; color: white; font-size: 2rem; } .dashboard-hero p { margin: .35rem 0 0; opacity: .92; }
    div[data-testid="stMetric"] { border-radius: 16px; padding: 1rem; background: linear-gradient(135deg,#eff6ff,#fdf2f8); border: 1px solid #ddd6fe; box-shadow: 0 4px 12px rgba(99,102,241,.10); }
    .section-card { border-radius: 16px; padding: .85rem 1rem; background: linear-gradient(135deg,#ecfeff,#f0fdf4); border-left: 6px solid #06b6d4; margin: .8rem 0; }
    </style>
    """, unsafe_allow_html=True)
    st.title(f"{APP_TITLE} 🚀")
    st.sidebar.header("Settings & Storage")
    mode = st.sidebar.selectbox("Storage backend", ["sqlite", "json"], index=0 if DEFAULT_STORAGE == "sqlite" else 1)
    storage = Storage(mode); state = storage.get_state()
    ...
    with practice:
        ...
        if st.button("Run Tests", type="primary"):
            ...
            if "error" in result:
                ...
                for item in result["results"]: st.write(("✅" if item["passed"] else "❌") + f" Input: {item['input']} → Output: {item['output']} (expected {item['expected']})")
    ...
    with dashboard:
        state = storage.get_state(); display_name = state.get("preferred_name") or name or USER_NAME
        st.markdown(f'<div class="dashboard-hero"><h1>Welcome back, {display_name} 👋</h1><p>Your colorful Python learning dashboard · Keep building your streak!</p></div>', unsafe_allow_html=True)
        st.markdown('<div class="section-card">🌈 <strong>Your progress at a glance</strong> — celebrate every solved test.</div>', unsafe_allow_html=True)
        a, b, c = st.columns(3); a.metric("🏆 Level", state["level"]); b.metric("🔥 Daily streak", state["streak"]); c.metric("✨ Success streak", state["success_streak"])
        ...
        if attempts:
            ...
            chart = alt.Chart(summary).mark_area(line={"color": "#7c3aed"}, color=alt.Gradient gradient="linear", stops=[alt.GradientStop(color="#c4b5fd", offset=0), alt.GradientStop(color="#fbcfe8", offset=1)], orient="vertical").encode(x=alt.X("date:T", title="Date"), y=alt.Y("pass_rate:Q", title="Pass Rate", scale=alt.Scale(domain=[0, 1])), tooltip=["date", "count", "passed", "pass_rate"]).properties(height=300)
            st.altair_chart(chart, use_container_width=True); st.dataframe(frame.head(20)[["date", "task_id", "passed"]], use_container_width=True, hide_index=True); st.progress(float(frame["passed"].mean()), text=f"Overall success rate: {frame['passed'].mean():.0%}")
        else: st.info("No attempts yet. Start practicing to bring your dashboard to life! 🚀")
    with settings:
